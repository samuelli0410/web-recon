import argparse
import numpy as np
import scipy
import scipy.ndimage
import matplotlib.pyplot as plt

from utils import load_pcd_array, calculate_density

def p_average(density, p=1):
    """
    Infers density for each block based on its neighbors using p-power mean with given p.
    """
    print(f"Inferring density for each block based on its neighbors as p-power mean with p = {p:.4f}")
    
    # Kernel for averaging densities of 6 neighboring blocks (3D cross-shaped kernel)
    kernel = np.zeros((3, 3, 3))
    kernel[0, 1, 1] = 1/6  # Neighbor in -x direction
    kernel[2, 1, 1] = 1/6  # Neighbor in +x direction
    kernel[1, 0, 1] = 1/6  # Neighbor in -y direction
    kernel[1, 2, 1] = 1/6  # Neighbor in +y direction
    kernel[1, 1, 0] = 1/6  # Neighbor in -z direction
    kernel[1, 1, 2] = 1/6  # Neighbor in +z direction

    if p > 0:
        density_p = np.power(density, p)
        inferred_density_p = scipy.ndimage.convolve(density_p, kernel, mode="constant", cval=0.0)
        inferred_density = np.clip(inferred_density_p, 0, None) ** (1/p)
    else:
        # Geometric mean when p = 0: exp(avg(log(density)))
        eps = 1e-6  # Small value to avoid log(0)
        density_log = np.log(density + eps)
        inferred_density_log = scipy.ndimage.convolve(density_log, kernel, mode="constant", cval=0.0)
        inferred_density = np.exp(inferred_density_log)
    return inferred_density

def p_harmonicity(density, p=1, q=1):
    """
    Computes the error between density and p-averaged density using averaged L^q norm.
    """
    assert q > 0, "q must be positive"
    depth, height, width = density.shape
    p_average_density = p_average(density, p)
    # Exclude boundary voxels where convolution might be affected
    err = np.abs(density - p_average_density)[1: depth - 1, 1: height - 1, 1: width - 1]
    return np.mean(err ** q) ** (1/q)

def plot_error_vs_p(density, q=1, delta=0.01, max_p=3.0):
    """
    Plots the error between actual and p-averaged densities over a range of p values.
    """
    p_range = np.arange(0.0 + delta, max_p + delta, delta)
    errors = [p_harmonicity(density, p, q) for p in p_range]
    best_p = p_range[np.argmin(errors)]
    print(f"Best p: {best_p}")
    plt.plot(p_range, errors)
    plt.xlabel("p")
    plt.ylabel(f"L^{q} error")
    plt.title(f"L^{q} error vs p")
    plt.show()

def find_brightest_slice(density):
    """
    Finds the slice with the highest total density along the depth axis.
    """
    # Sum densities over x and y axes for each depth (z-axis)
    slice_sums = np.sum(density, axis=(1, 2))
    brightest_slice_idx = np.argmax(slice_sums)
    return brightest_slice_idx

def visualize_inferred_vs_actual(actual_density, inferred_density, slice_idx=None):
    """
    Visualizes a specific slice of actual and inferred densities side by side.
    """
    fig, axes = plt.subplots(1, 2, figsize=(12, 6))

    if slice_idx is None:
        slice_idx = find_brightest_slice(actual_density)
    
    # Plot actual densities for a specific slice
    axes[0].imshow(actual_density[slice_idx, :, :], cmap='Greys', interpolation='nearest')
    axes[0].set_title("Actual Density (Slice)")
    axes[0].axis('off')
    
    # Plot inferred densities for the same slice
    axes[1].imshow(inferred_density[slice_idx, :, :], cmap='Greys', interpolation='nearest')
    axes[1].set_title("Inferred Density (Slice)")
    axes[1].axis('off')
    
    plt.suptitle(f"Comparison of Actual and Inferred Densities at Depth Slice {slice_idx}")
    plt.show()

def compute_laplacian(density):
    """
    Computes the discrete Laplacian of the silk density grid.
    """
    print("Computing the discrete Laplacian of the silk density.")
    # Define the 3D Laplacian kernel
    laplacian_kernel = np.zeros((3, 3, 3))
    laplacian_kernel[1, 1, 1] = -6
    laplacian_kernel[0, 1, 1] = 1  # Neighbor in -x direction
    laplacian_kernel[2, 1, 1] = 1  # Neighbor in +x direction
    laplacian_kernel[1, 0, 1] = 1  # Neighbor in -y direction
    laplacian_kernel[1, 2, 1] = 1  # Neighbor in +y direction
    laplacian_kernel[1, 1, 0] = 1  # Neighbor in -z direction
    laplacian_kernel[1, 1, 2] = 1  # Neighbor in +z direction

    # Compute the Laplacian using convolution
    laplacian = scipy.ndimage.convolve(density, laplacian_kernel, mode='constant', cval=0.0)
    return laplacian

def harmonic_p_norm(laplacian, p=2):
    """
    Computes the harmonic p-norm of the Laplacian.
    """
    laplacian_flat = laplacian.flatten()
    norm = np.linalg.norm(laplacian_flat, ord=p)
    return norm

def visualize_laplacian(laplacian, slice_idx=None):
    """
    Visualizes a specific slice of the Laplacian.
    """
    if slice_idx is None:
        slice_idx = find_brightest_slice(np.abs(laplacian))
    
    plt.imshow(laplacian[slice_idx, :, :], cmap='seismic', interpolation='nearest')
    plt.title(f"Laplacian of Silk Density at Depth Slice {slice_idx}")
    plt.colorbar(label='Laplacian Value')
    plt.axis('off')
    plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Infer density from neighboring blocks and compute Laplacian.")
    parser.add_argument("--file_name", type=str, help="File name of the PCD file of a web (without extension).")
    parser.add_argument("--num_M", type=int, default=20, help="Number of subdivisions for density calculation. Default is 20.")
    parser.add_argument("--p", type=float, default=1.0, help="Power for p-power mean. Default is 1.0.")
    parser.add_argument("--q", type=float, default=1.0, help="Power for L^q norm. Default is 1.0.")
    parser.add_argument("--visualize", action="store_true", help="Visualize the inferred density vs original density.")
    parser.add_argument("--plot_p", action="store_true", help="Plot error vs p. Also find the best p.")
    parser.add_argument("--delta", type=float, default=0.01, help="Step size for p in the plot. Default is 0.01.")
    parser.add_argument("--max_p", type=float, default=3.0, help="Maximum p for the plot. Default is 3.0.")
    parser.add_argument("--compute_laplacian", action="store_true", help="Compute the Laplacian of the density.")
    parser.add_argument("--laplacian_p", type=float, default=2.0, help="Power for harmonic p-norm of Laplacian. Default is 2.0.")
    parser.add_argument("--visualize_laplacian", action="store_true", help="Visualize a slice of the Laplacian.")
    args = parser.parse_args()

    path = f"video_processing/point_clouds/!010 255 2024-10-31 18-38-44.pcd"
    num_M = args.num_M
    p = args.p
    q = args.q
    delta = args.delta
    max_p = args.max_p
    laplacian_p = args.laplacian_p

    # Load the point cloud data
    points = load_pcd_array(path)
    # Calculate the density grid
    density = calculate_density(points, M=num_M)
    # Compute the inferred density using p-average
    inferred_density = p_average(density, p)
    # Compute the harmonicity error
    q_harmonicity = p_harmonicity(density, p, q)
    print(f"L^{q} error: {q_harmonicity:.4f}")

    if args.visualize:
        slice_idx = num_M // 2 
        visualize_inferred_vs_actual(density, inferred_density, slice_idx=slice_idx)
    if args.plot_p:
        plot_error_vs_p(density, q=q, delta=delta, max_p=max_p)
    if args.compute_laplacian:
        laplacian = compute_laplacian(density)
        laplacian_norm = harmonic_p_norm(laplacian, p=laplacian_p)
        print(f"Harmonic {laplacian_p}-norm of the Laplacian: {laplacian_norm:.4f}")
        if args.visualize_laplacian:
            visualize_laplacian(laplacian)
